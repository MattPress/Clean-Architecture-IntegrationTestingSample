# IntegrationTestingSample: The Clean Architecture Sample Implementation with .NET Core :cyclone:
[![All Contributors](https://img.shields.io/badge/all_contributors-12-orange.svg?style=flat-square)](#contributors)
[![Build status](https://ci.appveyor.com/api/projects/status/0i6s33kw3y87tkb2?svg=true)](https://ci.appveyor.com/project/ivanpaulovich/clean-architecture-integrationtestingsample)<a href="https://www.nuget.org/packages/Paulovich.Caju/" rel="Paulovich.Caju">![NuGet](https://buildstats.info/nuget/paulovich.caju)</a> [![Build Status](https://travis-ci.org/ivanpaulovich/dotnet-new-caju.svg?branch=master)](https://travis-ci.org/ivanpaulovich/dotnet-new-caju)
...
dotnet new integrationtestingsample -n "MyGreatApi"
```

**ProTip #1:** To get the Clean Architecture updates hit the `WATCH` button :eyes:.

**IntegrationTestingSample** is a virtual Wallet application in which a customer can register an account then manage the balance with `Deposits`, `Withdraws` and `Transfers`.

The IntegrationTestingSample's demo is hosted on `Heroku` servers and the `Swagger UI` client is available at [https://clean-architecture-integrationtestingsample.herokuapp.com/swagger/](https://clean-architecture-integrationtestingsample.herokuapp.com/swagger/). It is just beautiful!
[![Swagger Demo](https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-integrationtestingsample/master/docs/clean-architecture-integrationtestingsample-swagger.png)](https://clean-architecture-integrationtestingsample.herokuapp.com/swagger/index.html)
...
<p align="center">
  ...
  <br>
  <br>
  <a href="https://labs.play-with-docker.com/?stack=https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-integrationtestingsample/master/docker-compose.yml&amp;stack_name=clean-architecture-integrationtestingsample" rel="nofollow"><img src="https://raw.githubusercontent.com/play-with-docker/stacks/master/assets/images/button.png" alt="Try in PWD" style="max-width:100%;"></a>
</p>
...
## Index of Clean Architecture IntegrationTestingSample
...
Application architecture is about usage, a good architecture screams the business use cases to the developer and framework concerns are implementation details. On **IntegrationTestingSample** sample the user can `Register` an account then manage the balance by `Deposits`, `Withdrawals` and `Transfers`.

<p align="center">
  <img src="https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-integrationtestingsample/master/docs/clean-architecture-integrationtestingsample-use-cases.png" alt=Clean Architecture Use Cases" style="max-width:100%;">
</p>
...
![Register Flow of Control](https://github.com/ivanpaulovich/clean-architecture-integrationtestingsample/blob/master/docs/register-flow-of-control.svg)
...
IntegrationTestingSample uses ideas from popular architectural styles. They Ports and Adapters are the simplest one followed by the others, they complement each other and aim a software made by use cases decoupled from technology implementation details.
...
![Ports and Adapters](https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-integrationtestingsample/master/docs/clean-architecture-integrationtestingsample-ports-and-adapters.png)
...
<p align="center">
  <img src="https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-integrationtestingsample/master/docs/clean-architecture-integrationtestingsample-layers.png" alt="Layers" style="max-width:100%;">
</p>
...
namespace IntegrationTestingSample.WebApi.Extensions
{
    ...
    using IntegrationTestingSample.WebApi.Filters;
    ...
    public static class VersionedSwaggerExtensions
    {
            ...
            ApiVersionDescription apiVersion)
        {
            ...
            options.SwaggerDoc(apiVersion.GroupName,
                new Info()
                {
                    Title = "Clean Architecture IntegrationTestingSample",
                        Contact = new Contact()
                        {
                            Name = "@ivanpaulovich",
...
Data Annotations are powerful tool from .NET, it can be interpreted by ASP.NET Core and other frameworks to generate Validation, User Interface and other things. On IntegrationTestingSample project, Data Annotations are used to create a complete Swagger UI and HTTP Request validation. Of course following the Clean Architecture Principles we need to keep frameworks under control.
...
```c#
public sealed class IntegrationTestingSampleContext : DbContext
{
    public IntegrationTestingSampleContext(DbContextOptions options) : base(options)
    {
...
Run the EF Tool to add a migration to the `IntegrationTestingSample.Infrastructure` project.

```sh
dotnet ef migrations add "InitialCreate" -o "EntityFrameworkDataAccess/Migrations" --project source/IntegrationTestingSample.Infrastructure --startup-project source/IntegrationTestingSample.WebApi
```
...
```sh
dotnet ef database update --project source/IntegrationTestingSample.Infrastructure --startup-project source/IntegrationTestingSample.WebApi
```
...
```sh
dotnet run --project "source/IntegrationTestingSample.WebApi/IntegrationTestingSample.WebApi.csproj" --Environment="Development"
```
...
```sh
dotnet run --project "source/IntegrationTestingSample.WebApi/IntegrationTestingSample.WebApi.csproj" --Environment="Production"
```
...
IntegrationTestingSample is a cross-platform application, you can run it from Mac, Windows or Unix. To develop new features, you may use Visual Studio or Visual Studio Code :heart:.
...
```sh
dotnet run --project "source/IntegrationTestingSample.WebApi/IntegrationTestingSample.WebApi.csproj"
```
...
build_script:
  ...
  - dotnet ef database update --project source/IntegrationTestingSample.Infrastructure --startup-project source/IntegrationTestingSample.WebApi
  - dotnet build
  - pushd source/IntegrationTestingSample.WebApi/
  - dotnet pack --configuration Release
  - popd
test_script:
  - dotnet test tests/IntegrationTestingSample.UnitTests/IntegrationTestingSample.UnitTests.csproj
  - dotnet test tests/IntegrationTestingSample.IntegrationTests/IntegrationTestingSample.IntegrationTests.csproj
  - dotnet test tests/IntegrationTestingSample.AcceptanceTests/IntegrationTestingSample.AcceptanceTests.csproj
deploy_script:
  - docker build -t ivanpaulovich/clean-architecture-integrationtestingsample:github .
  - docker login -u="$DOCKER_USER" -p="$DOCKER_PASS"
  - docker push ivanpaulovich/clean-architecture-integrationtestingsample:github
  - docker login --username=$HEROKU_USERNAME --password=$HEROKU_API_KEY registry.heroku.com
  - docker tag ivanpaulovich/clean-architecture-integrationtestingsample:github registry.heroku.com/$HEROKU_APP_NAME/web
...
WORKDIR /app
COPY --from=build-env /app/source/IntegrationTestingSample.WebApi/out .
CMD export ASPNETCORE_URLS=http://*:$PORT && dotnet IntegrationTestingSample.WebApi.dll
```
...
| [Clean Architecture IntegrationTestingSample](https://www.youtube.com/watch?v=ivAkdJmSqLQ)              | Aug 6, 2019  |
